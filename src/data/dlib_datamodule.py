...
class DlibDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return cropped_image, landmarks  # chua chuan hoa
    ...
    def _load_data(self, data_dif: str, xml_file: str):
        ...
        samples = [self._get_cropped_labeled_sample(sample) for sample in samples]
        return samples
    ...
    def _get_cropped_labeled_sample(self, sample: ET.Element) -> Dict:
        ...
        landmarks -= np.array([box_left, box_top])  # scare keypoint de phu hop voi anh
        return dict(
            filename=filename, width=width, height=height, box_top=box_top, box_left=box_left, box_width=box_width,
            box_height=box_height, landmarks=landmarks
        )
    ...
    def annotate_image(image: Image, landmark: np.ndarray) -> Image:
        ...
        for i in range(landmark.shape[0]):
            draw.ellipse((landmark[i, 0] - 2, landmark[i, 1] - 2,
                          landmark[i, 0] + 2, landmark[i, 1] + 2), fill=(255, 255, 0))
        return image


class TransformDataset(Dataset):
    def __init__(self, dataset: DlibDataset, transfrom: Optional[Compose] = None):
    ...
    def __getitem__(self, idx):
        ...
        return image, landmarks.astype(np.float32)
